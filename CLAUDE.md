...
- `favicon.svg` is the source of truth for the icon; `favicon-32.png`, `apple-touch-icon.png` and `og-image.png` were rendered from it (and from the hero layout) by a one-off Pillow script. Change the SVG and the PNGs are stale — regenerate rather than hand-editing them.
